...
public class CreditScoreRules {
	...
	public static int getCreditScore(CreditScoreInputs crScoreIn) {
		...
		logger.debug("CompanyType()" + crScoreIn.getCompanyType());

		if (crScoreIn.getCompanyType().toLowerCase().contentEquals("Sole Proprietorship")) {
			_creditScore += 12;
		} else if (crScoreIn.getCompanyType().toLowerCase().contentEquals("Limited Liability Company")) {
			_creditScore += 63;
		} else if (crScoreIn.getCompanyType().toLowerCase().contentEquals("Partnership")) {
			_creditScore += 75;
		} else if (crScoreIn.getCompanyType().toLowerCase().contentEquals("Others")) {
			_creditScore += 0;
		} else if (crScoreIn.getCompanyType().isEmpty()) {
			_creditScore += 0;
		} else {
			_creditScore += 0;
		}

		logger.debug("NumberOfEmployees()" + crScoreIn.getNumberOfEmployees());
		...
		logger.debug("TimeInBusiness()" + crScoreIn.getTimeInBusiness());
